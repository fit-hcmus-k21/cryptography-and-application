#include <bits/stdc++.h>
using namespace std;

// ------------------------------------AES encryption-----------------------------------------------
const unsigned char SBox[16][16] =
    {
        {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76},
        {0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0},
        {0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15},
        {0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75},
        {0x09, 0x83, 0x2C, 0x1A, 0x1B, 0x6E, 0x5A, 0xA0, 0x52, 0x3B, 0xD6, 0xB3, 0x29, 0xE3, 0x2F, 0x84},
        {0x53, 0xD1, 0x00, 0xED, 0x20, 0xFC, 0xB1, 0x5B, 0x6A, 0xCB, 0xBE, 0x39, 0x4A, 0x4C, 0x58, 0xCF},
        {0xD0, 0xEF, 0xAA, 0xFB, 0x43, 0x4D, 0x33, 0x85, 0x45, 0xF9, 0x02, 0x7F, 0x50, 0x3C, 0x9F, 0xA8},
        {0x51, 0xA3, 0x40, 0x8F, 0x92, 0x9D, 0x38, 0xF5, 0xBC, 0xB6, 0xDA, 0x21, 0x10, 0xFF, 0xF3, 0xD2},
        {0xCD, 0x0C, 0x13, 0xEC, 0x5F, 0x97, 0x44, 0x17, 0xC4, 0xA7, 0x7E, 0x3D, 0x64, 0x5D, 0x19, 0x73},
        {0x60, 0x81, 0x4F, 0xDC, 0x22, 0x2A, 0x90, 0x88, 0x46, 0xEE, 0xB8, 0x14, 0xDE, 0x5E, 0x0B, 0xDB},
        {0xE0, 0x32, 0x3A, 0x0A, 0x49, 0x06, 0x24, 0x5C, 0xC2, 0xD3, 0xAC, 0x62, 0x91, 0x95, 0xE4, 0x79},
        {0xE7, 0xC8, 0x37, 0x6D, 0x8D, 0xD5, 0x4E, 0xA9, 0x6C, 0x56, 0xF4, 0xEA, 0x65, 0x7A, 0xAE, 0x08},
        {0xBA, 0x78, 0x25, 0x2E, 0x1C, 0xA6, 0xB4, 0xC6, 0xE8, 0xDD, 0x74, 0x1F, 0x4B, 0xBD, 0x8B, 0x8A},
        {0x70, 0x3E, 0xB5, 0x66, 0x48, 0x03, 0xF6, 0x0E, 0x61, 0x35, 0x57, 0xB9, 0x86, 0xC1, 0x1D, 0x9E},
        {0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 0x9B, 0x1E, 0x87, 0xE9, 0xCE, 0x55, 0x28, 0xDF},
        {0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0x42, 0x68, 0x41, 0x99, 0x2D, 0x0F, 0xB0, 0x54, 0xBB, 0x16}};

const unsigned int Table_C[11] = {0x00000000, 0x01000000, 0x02000000, 0x04000000,
                                  0x08000000, 0x10000000, 0x20000000, 0x40000000,
                                  0x80000000, 0x1b000000, 0x36000000};

// func to convert 4 bytes -> 1 int
int BytesToInt(unsigned char src[])
{
    int dest = 0x00000000;
    for (int i = 0; i < 4; i++)
    {
        dest = (dest << 8) | src[i];
    }
    return dest;
}

// func to convert 1 int -> 4 bytes
void IntToBytes(int src, unsigned char *dest)
{
    for (int i = 0; i < 4; i++)
    {
        dest[i] = (src >> ((3 - i) * 8)) | 0;
    }
}

int RotateLeft(int src)
{
    unsigned char temp = src >> 24;
    src = (src << 8) | temp;
    return src;
}

int GetSBoxValue(int src)
{
    unsigned char *bytes_arr = new unsigned char[4];
    IntToBytes(src, bytes_arr);

    for (int i = 0; i < 4; i++)
    {
        unsigned char row = bytes_arr[i] >> 4;
        unsigned char col = bytes_arr[i] & 0b00001111;
        bytes_arr[i] = SBox[row][col];
    }

    int dest = BytesToInt(bytes_arr);
    delete[] bytes_arr;
    return dest;
}

unsigned char **KeyExpand(unsigned char key[])
{
    // convert key into 4 int
    int K[11][4];
    for (int i = 0; i < 4; i++)
    {
        K[0][i] = BytesToInt(key + i * 4);
    }

    // start loop
    for (int i = 1; i < 11; i++)
    {
        int temp = RotateLeft(K[i - 1][3]);
        K[i][0] = K[i - 1][0] ^ GetSBoxValue(temp) ^ Table_C[i];
        K[i][1] = K[i - 1][1] ^ K[i][0];
        K[i][2] = K[i - 1][2] ^ K[i][1];
        K[i][3] = K[i - 1][3] ^ K[i][2];
    }

    // covert to bytes
    unsigned char **Keys = new unsigned char *[11];
    for (int i = 0; i < 11; i++)
    {
        Keys[i] = new unsigned char[16];
        for (int j = 0; j < 4; j++)
        {
            IntToBytes(K[i][j], Keys[i] + j * 4);
        }
    }

    return Keys;
}

// ----------------------------------------End Key Expand-----------------------------------------

// Encryption: Multiply by 2 for MixColumns
unsigned char mul2[] =
    {
        };

// Encryption: Multiply by 3 for MixColumns
unsigned char mul3[] =
    {
        };

void AddRoundKey(unsigned char *state, unsigned char *roundKey)
{
    for (int i = 0; i < 16; i++)
    {
        state[i] ^= roundKey[i];
    }
}

void SubBytes(unsigned char *state)
{
    for (int i = 0; i < 16; i++)
    {
        state[i] = GetSBoxValue(state[i]);
    }
}

// Shift left, adds diffusion
void ShiftRows(unsigned char *state)
{
    unsigned char tmp[16];

    /* Column 1 */
    tmp[0] = state[0];
    tmp[1] = state[5];
    tmp[2] = state[10];
    tmp[3] = state[15];

    /* Column 2 */
    tmp[4] = state[4];
    tmp[5] = state[9];
    tmp[6] = state[14];
    tmp[7] = state[3];

    /* Column 3 */
    tmp[8] = state[8];
    tmp[9] = state[13];
    tmp[10] = state[2];
    tmp[11] = state[7];

    /* Column 4 */
    tmp[12] = state[12];
    tmp[13] = state[1];
    tmp[14] = state[6];
    tmp[15] = state[11];

    for (int i = 0; i < 16; i++)
    {
        state[i] = tmp[i];
    }
}

void MixColumns(unsigned char *state)
{
    unsigned char tmp[16];

    tmp[0] = (unsigned char)mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3];
    tmp[1] = (unsigned char)state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3];
    tmp[2] = (unsigned char)state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]];
    tmp[3] = (unsigned char)mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]];

    tmp[4] = (unsigned char)mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7];
    tmp[5] = (unsigned char)state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7];
    tmp[6] = (unsigned char)state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]];
    tmp[7] = (unsigned char)mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]];

    tmp[8] = (unsigned char)mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11];
    tmp[9] = (unsigned char)state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11];
    tmp[10] = (unsigned char)state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]];
    tmp[11] = (unsigned char)mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]];

    tmp[12] = (unsigned char)mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15];
    tmp[13] = (unsigned char)state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15];
    tmp[14] = (unsigned char)state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]];
    tmp[15] = (unsigned char)mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]];

    for (int i = 0; i < 16; i++)
    {
        state[i] = tmp[i];
    }
}

void Round(unsigned char *state, unsigned char *key)
{
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, key);
}

// Same as Round() except it doesn't mix columns
void FinalRound(unsigned char *state, unsigned char *key)
{
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, key);
}

void AESEncrypt(unsigned char *plaintext, unsigned char **subKeys, unsigned char *ciphertext)
{
    unsigned char state[16]; // Stores the first 16 bytes of original message

    for (int i = 0; i < 16; i++)
    {
        state[i] = plaintext[i];
    }

    int numberOfRounds = 9;

    AddRoundKey(state, subKeys[0]); // Initial round

    for (int i = 0; i < numberOfRounds; i++)
    {
        Round(state, subKeys[i + 1]);
    }

    FinalRound(state, subKeys[10]);

    // Copy encrypted state to buffer
    for (int i = 0; i < 16; i++)
    {
        ciphertext[i] = state[i];
    }
}

// ------------------------------------------------------------------------------------------------
void CopyUnsignedCharBytes(unsigned char *dest, unsigned char *src, int n)
{
    for (int i = 0; i < n; i++)
    {
        dest[i] = src[i];
    }
}

// encrypt func : using AES 128bit enc
unsigned char *CBCRandomEnc(unsigned char *message, unsigned char *key, unsigned char *iv)
{
    // number block of plaintext 128bit
    int num_block = (strlen((const char *)message)) / 16;

    // ciphertext
    unsigned char **C = new unsigned char *[num_block + 1];
    for (int i = 0; i <= num_block; i++)
    {
        C[i] = new unsigned char[17];
    }

    // C[0] <- IV
    CopyUnsignedCharBytes(C[0], iv, 16);

    // loop
    unsigned char plaintext[17];
    unsigned char cipher[17];
    unsigned char **SubKeys = KeyExpand(key);

    for (int i = 1; i <= num_block; i++)
    {
        // C[i] <- Ek(C[i-1] xor M[i])
        for (int j = 0; j < 16; j++)
        {
            plaintext[j] = C[i - 1][j] ^ message[(i - 1) * 16 + j];
        }

        AESEncrypt(plaintext, SubKeys, cipher);

        CopyUnsignedCharBytes(C[i], cipher, 16);
    }

    // C <- C[1]....C[m]
    unsigned char *ciphertext = new unsigned char[16 * num_block + 1];
    for (int j = 0; j < num_block; j++)
    {
        CopyUnsignedCharBytes(ciphertext + j * 16, C[j + 1], 16);
    }

    // free mem
    for (int i = 0; i < 11; i++)
    {
        delete[] SubKeys[i];
    }

    return ciphertext;
}

int main()
{
    unsigned char message[] = "Two One Nine Two";
    unsigned char *message_encrypt = NULL;
    unsigned char key[] = "Thats my Kung Fu";
    unsigned char iv[] = "encryptionIntVec";

    // unsigned char message[] = "Two One Nine Two Ma Hoa Ung Dung";
    // unsigned char *message_encrypt = NULL;
    // unsigned char key[] = "Thats my friends";
    // unsigned char iv[] = "decryptionIVhihi";

    cout << "Message: " << message << endl;

    // print string message hex number
    message_encrypt = CBCRandomEnc(message, key, iv);
    cout << "\nCiphertext: \n";
    for (int i = 0; i < strlen( (char*)message); i++)
    {
        if (i % 16 == 0)
        {
            cout << endl;
        }
        printf("%02x ", (unsigned short) message_encrypt[i]);
    }
    cout << endl;

    // print string m_e hex number
    // message = CBCRandomDec(message_encrypt, key);

    // free mem
    delete[] message_encrypt;

    return 22;
}