...
const unsigned char Rcon[0x100] =
    {
...
const unsigned char s_fwd [0x100] =
	{
	  0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
	  0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
	  0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
...
void RotWord(unsigned char *word)
{
    ...
    // left-shift row unsigned char-wise once
    temp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
...
unsigned char **KeyExpand(unsigned char *key)
{
    ...
    int nBlock = 4;
    int nRound = 10;
    unsigned char w[4 * nBlock * (nRound + 1)];
    ...
    for (i = 4; i < nBlock * (nRound + 1); i++)
    {
        ...
        temp[3] = w[4 * (i - 1) + 3];
        if (i % 4 == 0)
        {                                    // perform every 4th 32-bit block...
            RotWord(temp);                   // unsigned char-wise rotation
            SubWord(temp);                   // substitution using S-box
            temp[0] = temp[0] ^ Rcon[i / 4]; // XOR left-most unsigned char with Rcon-element
        }

        w[4 * i] = w[4 * (i - 4)] ^ temp[0]; // XOR with a previous 32-bit block...
